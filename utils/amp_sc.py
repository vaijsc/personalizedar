...
class AmpOptimizer:
    def __init__(
        ...
        if self.enable_amp:
            ...
            self.scaler = (
                torch.cuda.amp.GradScaler(init_scale=2.0**11, growth_interval=1000)
                if self.using_fp16_rather_bf16
                else None
            )  # only fp16 needs a scaler
        else:
            self.amp_ctx = NullCtx()
            self.scaler = None

        self.optimizer, self.names, self.paras = (
            optimizer,
            names,
            paras,
        )  # paras have been filtered so everyone requires grad
        self.grad_clip = grad_clip
        self.early_clipping = self.grad_clip > 0 and not hasattr(
            optimizer, "global_grad_norm"
        )
        self.late_clipping = self.grad_clip > 0 and hasattr(
            optimizer, "global_grad_norm"
        )
