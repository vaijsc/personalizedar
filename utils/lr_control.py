...
def filter_params(model, nowd_keys=(), select_params=[]) -> Tuple[
    List[str],
    ...
    for name, para in model.named_parameters():
        ...
        if not (select_params[0] in name):
            continue
        
        print(select_params[0], select_params[0] in name)
        print(f"select_param: {name}")
        ...
        if name.endswith("bias") or any(k in name for k in nowd_keys):
